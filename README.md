...
- 稳定 Release 更新：区分热更新、普通更新和强制更新，并在更新前备份本地数据
...
- 远端 Supabase 记忆写入在后台进行，不会阻塞收到 QQ 消息和开始回复；最近的上下文会短暂缓存，减少连续聊天时的重复请求。
- 模型请求遇到临时网络错误、限流或 5xx 时会短暂重试；401、403、404 和参数错误不会盲目重试。
- 连续消息会合并后再交给模型。私聊批次有最长等待上限，最多收集 20 条，避免对方持续发消息时永远等不到回复。
- NapCat 重试上报同一条消息时，Bridge 会在短时间内去重，避免同一句触发两次回复；机器人自己发出的消息也不会再次触发回复。
- 启用 Supabase 时，远程历史和本地刚收到的消息会合并并按消息 ID 去重，远程写入短暂失败不会覆盖掉当前聊天上下文。
- 控制台会对模型生成的气泡做最后的格式兜底：去掉气泡内部换行、限制异常长气泡和过多气泡，并压低连续重复标点；正常括号、问号和省略号不会被统一改写。
- 控制台状态中的 NapCat 只有在 `get_status` API 实际成功时才显示“API 可用”。如果显示“端口已开但 API 不可用”，优先检查 Access Token 和 NapCat HTTP Server 配置。
- “备份配置”会保存本地 `.env.local`、预设、主题、Persona、表情词典和项目目录内的 SQLite 文件；备份包含密钥，请只放在自己的电脑上，不要上传 GitHub。
- Bridge 的 `/health` 会额外返回事件计数和批处理失败数，排查“收到了但没有回复”时可以先访问 `http://127.0.0.1:8766/health`。
