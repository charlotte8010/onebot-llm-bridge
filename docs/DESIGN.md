...
同一会话的同一 `message_id` 在短时间内只接受一次。这个去重既防止 NapCat 重试造成重复回复，也不依赖模型层自行判断。若事件明确标记为机器人自己发送的消息，则只记录、不进入回复流程。
...
如果启用了远程记忆，远程历史不会覆盖本地刚收到的上下文；两者按消息 ID 合并后再截取最近窗口。这样 Supabase 写入延迟或暂时不可用时，当前对话仍然优先。
