...
如果项目是用 `git clone` 下载的，现在可以直接在控制台右上角点击“检查更新”；发现稳定 Release 后，控制台会弹窗询问是否立即更新。普通的开发提交不会直接推给用户。控制台会：

1. 只读取版本清单和稳定 Release，不把每一次开发 `push` 都当成用户更新。
2. 先检查工作区；有本地代码改动、未推送提交或不是 Git 克隆目录时，会停止，不会替你删除文件。
3. 更新前自动备份 `.env.local`、Persona、词典、模型预设和 SQLite 记忆。
4. 只执行稳定版本的快进更新，不会制造合并提交，也不会覆盖本地数据。`hot` 版本只重启 Bot/Bridge，`normal` 版本会询问是否重启，`force` 版本会明确要求完成更新。
5. 如果控制台本身也更新了，服务重启后再关闭并重新打开控制台。
...
没有控制台时，也可以手动更新 Git 克隆，但要更新到发布者指定的稳定 Tag，不要直接跟随开发分支：
